...
const 使い方の表 = {
  ...
  '探す': '探す("名前")',
  'クローン': 'クローン(もの)',
  'プレイヤーの向き': 'プレイヤーの向き()  ※狙った方へ飛ばしたいときは カメラの向き() を使ってください',
};
...
export class Game {
  ...
  _じどうそうさ(dt) {
    ...
    if (l > 0.001) {
      p.__vx = (wx / l) * はやさ;
      p.__vz = (wz / l) * はやさ;
      // 歩いたら 体は 進行方向を むく（ロブロックスの三人称と同じ）。
      // 急に カクッと 向かないように、少しずつ まわします
      ...
      p.__vz = 0;
      // 止まっているときは 何もしません。
      // 使う人が 向ける() で入れた向きは、次に歩くまで そのまま のこります
    }
  ...
  // -------------------------------------------------------------------------
  // プレイヤーの「向き」と カメラの向き は 別もの
  //
  //   ロブロックスの三人称と同じで、**体の向き** と **見ている方向** を分けます。
  //
  //     カメラの向き()                … 見ている方向。ドラッグで変わる。読むだけ
  //     プレイヤー.向き / プレイヤーの向き() … 体が向いている方向
  //
  //   ● なぜ分けるか
  //       いちど「向き ＝ 見ている方向」に まとめてみたのですが、それだと
  //       向ける(プレイヤー, 90) でカメラも一緒に90度まわってしまい、
  //       カメラが体の真後ろに張りついたまま。画面では何も変わらないので、
  //       使う人には「キャラの向きが変えられない」と映りました。
  //       体だけ回せることのほうが 大事なので、2つに分けます。
  //
  //   ● 誰が 体の向きを 変えるか
  //       ・歩いたら 進行方向へ（_じどうそうさ の中。少しずつ まわす）
  //         横歩き・後ろ歩きでも 進行方向を向きます
  //       ・コードで 向ける(プレイヤー, 90) / プレイヤー.向き = 90 / 回す()
  //         → **体だけ** まわります。カメラは動きません
  //       ・止まっているあいだは そのまま。入れた向きは 次に歩くまで のこります
  //       ・一人称のときだけ 体＝カメラ（自分の体は見えないので）
  //
  //   ● 前へ(プレイヤー, n) は 「向き」（体の向き）のほう。
  //     ほかの もの と 同じ意味になるので、ここは 変えていません。
  //     ねらった方へ 弾を飛ばしたいときは カメラの向き() を つかいます。
  // -------------------------------------------------------------------------
  ...
  /** 今の向きから ねらいの向きへ、最大 `はやさ` 度だけ 近づける */
  _むきをちかづける(いま, ねらい, はやさ) {
    const い = 数にする(いま, 0);
    const ね = 数にする(ねらい, 0);
    if (!Number.isFinite(い)) return ね;
    const さ = ((ね - い + 540) % 360) - 180;
    if (Math.abs(さ) <= はやさ) return ね;
    return い + Math.sign(さ) * はやさ;
  }

  /**
   * 一人称のときだけ、体を カメラと 同じ向きにする。
   * 自分の体は 見えないので、分けても うれしくありません。
   * こうしておくと 前へ(プレイヤー, n) が 見ている方向へ すすみます。
   * カメラを すすめた あとに 呼びます（1フレーム おくれないように）。
   */
  _一人称なら体もカメラへ() {
    if (this._カメラモード !== 'いちにんしょう') return;
    const p = this.プレイヤー;
    if (!p || !p.__いきてる) return;
    if (this._カメラのあいて !== p) return;
    p.むき = this._むきをそろえる(this._カメラむき);
  }
  ...
  builtins() {
    ...
    /**
     * プレイヤーの向き() プレイヤーの **体** が向いている左右の角度（度）。
     *   プレイヤー.向き と同じ値です（0以上 360未満 にそろえて返します）。
     *   歩くと 進行方向を向き、向ける(プレイヤー, 90) で 体だけ まわせます。
     *   ※ 見ている方向は カメラの向き() です。ねらった方へ 弾を飛ばすときは
     *      向ける(たま, カメラの向き()) と書いてください。
     */
    const プレイヤーのむき = () => {
      const p = G.プレイヤー;
      if (!p || !p.__いきてる) return 0;
      return G._むきをそろえる(p.むき);
    };
